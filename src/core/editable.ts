...
export const editable = <T>(
  ...
  const flushInput = () => {
    ...
    if (queue.length) {
      ...
      const prevStartAndEnd = edges(...currentSelection);
      let deleteRange = prevStartAndEnd;
      const insertStart = prevStartAndEnd[0];
      const editRange = editedRange!;
      const isPrevSelectionCollapsed =
        comparePosition(...prevStartAndEnd) === 0;
      const isEditRangeCollapsed = comparePosition(...editRange) === 0;
      let isInsert = false;
      if (isPrevSelectionCollapsed && !isComposing) {
        if (isEditRangeCollapsed) {
          isInsert = true;
        } else {
          deleteRange = editRange;
        }
      } else {
        isInsert = true;
      }
      ...
      if (isInsert && comparePosition(insertStart, insertEnd) !== 0) {
        const domRange = getSelectionRangeInEditor(
        ...
        const updatedFragment = refToDoc(
          readDom(element, parserConfig, {
            ...
            _end: [domRange.endContainer, domRange.endOffset],
          }),
          serializeVoid
        );
